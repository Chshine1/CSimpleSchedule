...
public class UserService(IUnitOfWork unitOfWork, IMapper mapper) : IUserService
{
    public async Task<ApiResponse<UserDto>> RegisterAsync(UserDto userDto)
    {
        try
        {
            ...
            if (exists) return new ApiResponse<UserDto> { Status = false, Message = "Username already exists" };
            var user = mapper.Map<User>(userDto);
            await unitOfWork.GetRepository<User>().InsertAsync(user);
            return await unitOfWork.SaveChangesAsync() > 0
                ? new ApiResponse<UserDto> { Status = true, Result = mapper.Map<UserDto>(user) }
                : new ApiResponse<UserDto> { Status = false, Message = "Failed to register" };
        }
        catch (Exception e)
        {
